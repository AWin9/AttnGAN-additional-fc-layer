...
class RNN_ENCODER(nn.Module):
    def __init__(self, ntoken, ninput=300, drop_prob=0.5,
        ...
        self.n_hidden_rdc_text = int(ninput / 2)
    ...
    def define_module(self):
        ...
        self.rdc_text = nn.Linear(self.ninput, self.n_hidden_rdc_text)
        self.drop = nn.Dropout(self.drop_prob)
        if self.rnn_type == 'LSTM':
            # dropout: If non-zero, introduces a dropout layer on
            # the outputs of each RNN layer except the last layer
            self.rnn = nn.LSTM(self.n_hidden_rdc_text, self.nhidden,
            ...
            #                   dropout=self.drop_prob,
            #                   bidirectional=self.bidirectional)
            self.rnn = nn.GRU(self.n_hidden_rdc_text, self.nhidden,
                              self.nlayers, batch_first=True,
                              dropout=self.drop_prob,
                              bidirectional=self.bidirectional)
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        ...
        #emb_dropped = self.drop(emb)
        rdc_text = self.rdc_text(emb)
        # Returns: a PackedSequence object
        cap_lens = cap_lens.data.tolist()
        pack = pack_padded_sequence(rdc_text, cap_lens, batch_first=True)
        # #hidden and memory (num_layers * num_directions, batch, hidden_size):
        # tensor containing the initial hidden state for each element in batch.
        # #output (batch, seq_len, hidden_size * num_directions)
        # #or a PackedSequence object:
        # tensor containing output features (h_t) from the last layer of RNN
        output, hidden = self.rnn(pack, hidden)
